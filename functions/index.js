...
function validState(state, savedState) {
  if (!state || !savedState) return false;
  const a = Buffer.from(String(state));
  const b = Buffer.from(String(savedState));
  ...
  async (req, res) => {
    try {
      ...
      if (req.method === "GET" && path === "/discordCallback") {
        ...
        if (!validState(state, savedState)) return res.status(400).send("Invalid OAuth state.");
        ...
        if (!tokenData.access_token) return res.status(502).send("Discord did not return an access token.");
